...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8" id="properties-grid">
    @forelse($properties as $property)
        <article class="group relative bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
            ...
            <div class="relative aspect-[4/3] overflow-hidden">
                ...
                @else
                    <div class="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-gray-800">
                        <svg class="w-14 h-14 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l9-9 9 9v9a1 1 0 01-1 1h-5v-6h-6v6H4a1 1 0 01-1-1z" />
                        </svg>
                    </div>
                @endif

                {{-- Purpose Badge --}}
                <span class="absolute top-4 left-4 px-3 py-1 text-xs font-semibold rounded-full text-white
                    @if($property->purpose === 'buy') bg-green-600
                    @elseif($property->purpose === 'sell') bg-blue-600
                    @else bg-yellow-500 @endif">
                    {{ ucfirst($property->purpose) }}
                </span>

                {{-- Favorite --}}
                <button aria-label="Add to favorites" class="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/90 dark:bg-gray-800 flex items-center justify-center text-gray-400 hover:text-red-500 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Price --}}
                <div class="absolute bottom-4 left-4 bg-black/70 text-white px-4 py-1.5 rounded-lg text-sm font-semibold">
            ...
            <div class="p-6 space-y-4">
                ...
                {{-- CTA --}}
                <a
                    href="{{ route('buyer.properties.show', $property->id) }}"
                    class="block w-full text-center py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition">
                    View Details
                </a>
            </div>
        </article>
