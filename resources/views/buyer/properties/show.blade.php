                ...
                <div style="flex:2; min-width:0;">
                        ...
                        <div style="padding:1.75rem 2rem; border-top:1px solid #f0ece4;">
                            <div
                                ...
                                <div style="display:flex;gap:0.5rem;flex-shrink:0;">
                                    <button class="share-btn" onclick="shareOnFacebook()" title="Facebook">
                                        <svg viewBox="0 0 24 24" width="14" height="14" fill="#4267B2">
                                            <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" />
                                        </svg>
                                    </button>
                                    <button class="share-btn" onclick="shareOnTwitter()" title="Twitter">
                                        <svg viewBox="0 0 24 24" width="14" height="14" fill="#1DA1F2">
                                            <path
                                                d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z" />
                                        </svg>
                                    </button>
                                    <button class="share-btn" onclick="copyToClipboard()" title="Copy link">
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:0.5rem;margin-top:1rem;">
                                ...
                                @isset($property->category)
                                    <span class="type-badge">{{ ucfirst($property->category) }}</span>
                                @endisset
                    ...
                    <div class="detail-card" style="padding:2rem;">
                        ...
                        <div style="font-size:0.95rem;line-height:1.75;color:#4a4038;font-weight:300;">
                            <p>{{ Str::limit($property->description, 300) }}</p>
                        ...
                        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:1rem;">
                            ...
                            @if($property->area)
                                <div>
                                    ...
                                    <div style="font-size:1.1rem;font-weight:600;">{{ number_format($property->area) }} sqft</div>
                                </div>
                            @endif
                            <div>
                                <div style="font-size:0.65rem;color:#a09080;text-transform:uppercase;">Type</div>
                                <div style="font-size:1.1rem;font-weight:600;">{{ ucfirst($property->type) }}</div>
                            </div>
                            <div>
                                <div style="font-size:0.65rem;color:#a09080;text-transform:uppercase;">Purpose</div>
                                <div style="font-size:1.1rem;font-weight:600;">{{ $isBuy ? 'For Sale' : 'For Rent' }}</div>
                            </div>
                        </div>
                    </div>
                ...
                <div style="flex:0 0 22rem;min-width:0;width:100%;">
                    <div class="sidebar-sticky">
                        ...
                        <div class="price-card">
                            ...
                            @if(!$isBuy)
                                <div class="price-card-per">per month</div>
                            @endif
        ...
        Request Information
                            ...
                            @if($isBuy)
                                <div class="emi-card">
                                    ...
                                    <div class="emi-row">
                                        <span class="emi-key">Loan (80%)</span>
                                        <span class="emi-val">Rs {{ number_format($property->price * 0.8) }}</span>
                                    </div>
                                    <div class="emi-row">
                                        <span class="emi-key">Monthly EMI</span>
                                        <span class="emi-val highlight">Rs
                                            {{ number_format(($property->price * 0.8 * 0.00833) / (1 - pow(1 + 0.00833, -240))) }}
                                        </span>
                                    </div>
                        ...
                        <div class="prop-id-tag">Property ID
                            &nbsp;<span>#{{ str_pad($property->id, 8, '0', STR_PAD_LEFT) }}</span>
                        </div>
                        ...
                        <div class="seller-card">
                            ...
                            <div
                                style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.5rem;padding:0.875rem 0;
                                       border-top:1px solid #f0ece4;border-bottom:1px solid #f0ece4;
                                       margin-bottom:0.875rem;text-align:center;">
                            ...
                            <div style="display:flex;align-items:center;gap:0.375rem;margin-bottom:0.75rem;">
                                @for($i = 1; $i <= 5; $i++)
                                    <svg width="13" height="13" viewBox="0 0 20 20" fill="#c9a96e">
                        ...
                        <div class="safety-card">
                            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:1rem;">
                                ...
                                <span
                                    style="font-size:0.62rem;letter-spacing:0.14em;text-transform:uppercase;color:#c9a96e;font-weight:600;">
                                    Safety Tips
                                </span>
                            </div>
                            @foreach([
                                'Inspect the property in person before making any payment',
                                'Verify all ownership documents with a legal expert',
                                'Never pay in cash or via untraceable methods'
                            ] as $i => $tip)
                        ...
                        <div class="rv-card">
                            ...
                            @forelse($recentlyViewed as $recent)
                                <a href="{{ route('buyer.properties.show', $recent->id) }}" class="rv-item">
                                    <div class="rv-thumb">
                                        <img src="{{ $recent->image ? asset('storage/' . $recent->image) : asset('images/image1.jpg') }}"
                                             alt="{{ $recent->title }}">
                                    </div>
                                    <div style="flex:1;min-width:0;">
...
function showEnqToast(message, type = 'success') {
    alert(message); // replace with your toast UI
            ...
            function initMap() {
                if (!propLat || !propLng) {
                    document.getElementById('propertyMap').innerHTML =
                        "<p style='padding:1rem;text-align:center;color:#8c8070;'>Location not available</p>";
                    return;
                }
                ...
                const icon = L.divIcon({
                    ...
                    html: `<div style="width:36px;height:36px;border-radius:50% 50% 50% 0;
                                       transform:rotate(-45deg);background:#c9a96e;
            ...
            document.addEventListener('DOMContentLoaded', () => {
                if (document.getElementById('propertyMap')) initMap();
            });
        </script>
    @endpush
